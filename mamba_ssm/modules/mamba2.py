...
try:
    from mamba_ssm.ops.triton.layernorm_gated import RMSNorm as RMSNormGated  # type: ignore
except Exception:
    RMSNormGated = None
    ...
    def forward(self, x: torch.Tensor, z: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        if self.group_size is None:
            ...
            if x_float.shape[-1] % group_size != 0:
                raise ValueError(f"group_size={group_size} must divide dim={int(x_float.shape[-1])}")
            g = x_float.shape[-1] // group_size
            xg = x_float.reshape(*x_float.shape[:-1], g, group_size)
            rms = torch.rsqrt(xg.pow(2).mean(dim=-1, keepdim=True) + self.eps)
            y = (xg * rms).reshape_as(x_float) * self.weight.float()
...
class Mamba2(nn.Module, PyTorchModelHubMixin):
    def __init__(
        ...
        if self.cluster_config is not None:
            if self.process_group is not None:
                raise NotImplementedError("cluster_config is only supported when process_group is None")
    ...
    def _ssm_scan_naive(
        ...
        *,
        ...
        dt: torch.Tensor,  # (B, L, H) (raw or already softplus)
        A: torch.Tensor,  # (H,)
        B: torch.Tensor,  # (B, L, G, N)
        C: torch.Tensor,  # (B, L, G, N)
        D: torch.Tensor,  # (H,) or (H, P)
        z: Optional[torch.Tensor],  # (B, L, H, P) or None
        dt_bias: Optional[torch.Tensor],  # (H,) or None
        dt_softplus: bool,
        ...
        if dt_bias is not None:
            dt_work = dt_work + dt_bias.reshape(1, 1, -1).float()
        if dt_softplus:
            dt_work = F.softplus(dt_work)
        dt_work = dt_work.to(dtype=x.dtype)
        ...
        for t in range(seqlen):
            ...
            y_t = (state * C_t[:, :, None, :]).sum(dim=-1)
            if D.ndim == 1:
                y_t = y_t + D.reshape(1, -1, 1) * x_t
            else:
                y_t = y_t + D.reshape(1, nheads, headdim) * x_t
            if z is not None:
                y_t = y_t * F.silu(z[:, t, :, :].to(dtype=x.dtype))
            ys.append(y_t)
    ...
    def forward(self, u, seqlen=None, seq_idx=None, cu_seqlens=None, inference_params=None):
        ...
        if (
            ...
            and u.is_cuda
            and self.cluster_config is None
        ):
            ...
            D_param = rearrange(self.D, "(h p) -> h p", p=self.headdim) if self.D_has_hdim else self.D
            z_hp = rearrange(z, "b l (h p) -> b l h p", p=self.headdim) if not self.rmsnorm else None
            ...
            if mamba_chunk_scan_combined is not None and u.is_cuda and self.cluster_config is None:
                ...
                if cu_seqlens is not None:
                    raise NotImplementedError("cu_seqlens/varlen is not supported without Triton kernels")
                y, last_state = self._ssm_scan_naive(
                    x=x_hp,
                    dt=dt,
                    A=A,
                    B=B_g,
    ...
    def step(self, hidden_states, conv_state, ssm_state):
        ...
        # Conv step
        if self.cluster_config is not None or causal_conv1d_update is None:
        ...
        # SSM step
        if self.cluster_config is not None or selective_state_update is None:
            ...
            y = (ssm_state.to(dtype) * C[:, None, None, :]).sum(dim=-1)
            if self.D_has_hdim:
                D_hp = self.D.to(dtype).reshape(self.nheads, self.headdim)
                y = y + D_hp.unsqueeze(0) * x
            else:
                y = y + self.D.to(dtype).reshape(1, -1, 1) * x
