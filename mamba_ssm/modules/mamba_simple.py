...
class Mamba(nn.Module):
    ...
    def _cluster_weight_name(self, suffix: str) -> str:
        ...
        prefix = self.cluster_config.name_prefix if self.cluster_config is not None else "mamba"
    ...
    def forward(self, hidden_states, inference_params=None):
        ...
        if self.cluster_config is not None:
            ...
            xz = xz.transpose(1, 2).contiguous()  # (B, 2*d_inner, L)
        else:
        ...
        if self.use_fast_path and causal_conv1d_fn is not None and inference_params is None:  # Doesn't support outputting the states
            ...
            if self.cluster_config is not None or causal_conv1d_fn is None:
            ...
            if self.cluster_config is not None:
                x_dbl = cluster_linear(
                    x_flat,
                    self.x_proj.weight,
                    None,
                    cfg=self.cluster_config,
                    weight_name=self._cluster_weight_name("x_proj_weight"),
                )
            else:
                x_dbl = self.x_proj(x_flat)  # (bl d)
            dt, B, C = torch.split(x_dbl, [self.dt_rank, self.d_state, self.d_state], dim=-1)
            if self.cluster_config is not None:
                dt_out = cluster_linear(
                    dt,
                    self.dt_proj.weight,
                    None,  # dt bias is applied inside the scan (delta_bias)
                    cfg=self.cluster_config,
                    weight_name=self._cluster_weight_name("dt_proj_weight"),
                )  # (bl, d_inner)
                dt = dt_out.reshape(batch, seqlen, self.d_inner).transpose(1, 2).contiguous()
            else:
    ...
    def step(self, hidden_states, conv_state, ssm_state):
        ...
        if self.cluster_config is not None:
            ...
            dt = dt @ self.dt_proj.weight.t()  # (B d_inner)
        A = -torch.exp(self.A_log.float())  # (d_inner, d_state)
        ...
        if self.cluster_config is not None or selective_state_update is None:
            ...
            y = (ssm_state.to(dtype) * C.unsqueeze(1)).sum(dim=-1)
