...
try:
    from mamba_ssm.ops.triton.layer_norm import _layer_norm_fwd  # type: ignore
except Exception:
...
def rms_norm_forward(
    ...
    if _layer_norm_fwd is not None:
        ...
        return y
    # Fallback: pure PyTorch (CPU/any device)
    x_float = x.float()
    if is_rms_norm:
        rms = torch.rsqrt(x_float.pow(2).mean(dim=-1, keepdim=True) + eps)
        y = x_float * rms
    else:
        y = F.layer_norm(x_float, x_float.shape[-1:], weight=None, bias=None, eps=eps)
...
def selective_scan_ref(u, delta, A, B, C, D=None, z=None, delta_bias=None, delta_softplus=False,
    ...
    if not is_variable_B:
        ...
        if B.dim() == 3:
            # B: (B, N, L) -> (B, 1, L, N)
            deltaB_u = (delta * u).unsqueeze(-1) * B.permute(0, 2, 1).unsqueeze(1)
        else:
            B = repeat(B, "B G N L -> B (G H) N L", H=dim // B.shape[1])
            deltaB_u = (delta * u).unsqueeze(-1) * B.permute(0, 1, 3, 2)
